...
package object b3 {
  ...
  case class B3FieldInfo(field: Field, withFeedback: Boolean, withLabelFor: Boolean, args: Seq[(Symbol, Any)], messages: Messages) {
    ...
    def name: String = if (args.exists(_ == ('_appendIdWithName, true)) && !field.name.endsWith(".id")) { field.name + ".id" } else { field.name }

    /* Value of the input */
    //val value: Option[String] = field.value.orElse(argsMap.get('value).map(_.toString))
    val value: Option[String] = {
      if (argsMap.get('value) != None)
        argsMap.get('value).map(_.toString)
      else
        field.value
    }
    ...
    def isRequired: Boolean = B3FieldInfo.isRequired(field)
  ...
  object B3FieldInfo {
    ...
    def isRequired(field: Field): Boolean = {
      field.constraints.foreach { c =>
        //println("===============>" + c._1)
        if (c._1 == "constraint.required")
          return true
      }
